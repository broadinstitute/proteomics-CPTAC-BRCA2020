...
setwd('c:/Users/karsten/Dropbox/Devel/CPTAC-BRCA2020/')

## import gct files
data.dir <- 'g:/My Drive/CPTAC/prospBRCA/data/data_freeze_v5.4-public//'
label <- 'v5.4-public'

ord.column <- 'PAM50'

gct.str <- c(glue("{data.dir}prosp-brca-{label}-acetylome-ratio-norm-NArm.gct"),
             glue("{data.dir}prosp-brca-{label}-gene-level-cnv-gistic2-all_data_by_genes.gct"),
             glue("{data.dir}prosp-brca-{label}-phosphoproteome-ratio-norm-NArm.gct"),
             glue("{data.dir}prosp-brca-{label}-proteome-ratio-norm-NArm.gct"),
             glue("{data.dir}prosp-brca-{label}-rnaseq-fpkm-log2-row-norm-median-mad.gct")
             )
...
## export
save(column.anno, row.anno, tab.expr.all, file = glue('data/data-brca2020-{label}.Rdata'))
