...
shinyServer( function(input, output, session) {
      ...
      list(
        ...
        fluidRow(
            column(6, selectizeInput('sort.after', 'Sort by', 
                                     choices=c('PAM50', 
                                               'NMF.cluster', 
                                               'ERBB2.Proteogenomic.Status', 
                                               'PAM50.Her2.HER2.status',
                                               'HER2.Ampilified', 
    ...
    output$plot <- renderPlot({
     ...
     hm=makeHM(genes.vec, expr=tab.expr.all, column.anno=column.anno, row.anno=row.anno, zscore=input$zscore, show.sites=input$allsites, min.val=as.numeric(input$min.val), max.val=as.numeric(input$max.val), anno.class=input$sort.after, sort.dir=input$sort.dir)
    ...
    output$downloadHM <- downloadHandler(
        ...
        content = function(file){
          ...
          if(length(genes.vec)==0) return()
          #debug(makeHM)
          hm=try(makeHM(genes.vec, expr=tab.expr.all, column.anno=column.anno, row.anno=row.anno, zscore=input$zscore, show.sites=input$allsites, min.val=as.numeric(input$min.val), max.val=as.numeric(input$max.val),anno.class=input$sort.after, sort.dir=input$sort.dir, filename = file))
        }
     )       # hm=makeHM(extractGenes( input$genes ), expr=tab.expr.all, column.anno=column.anno, row.anno=row.anno, zscore=input$zscore,
